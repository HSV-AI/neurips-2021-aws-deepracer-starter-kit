...
class PPOLightning(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.critic = RacerNet(self.env.observation_space.shape, 1)
        # policy network (agent)
        if isinstance(self.env.action_space, gym.spaces.box.Box):
            act_dim = self.env.action_space.shape[0]
            # actor_mlp = create_mlp(self.env.observation_space.shape, act_dim)
            actor_mlp = RacerNet(self.env.observation_space.shape, act_dim)
            ...
            actor_mlp = RacerNet(self.env.observation_space.shape, self.env.action_space.n)
